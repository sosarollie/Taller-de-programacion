  ...
  begin
    if (a <> nil) then
		begin
			...
			if (maxCant < a^.dato.cantTotalUnidades) then begin
...
estructura. }
  ...
  begin
    if (a <> nil) and (a^.dato.codigo < unCodigo) then
		ObtenerCantidadCodigosMenores:= 1 + ObtenerCantidadCodigosMenores(a^.HI, unCodigo) + ObtenerCantidadCodigosMenores(a^.HD, unCodigo)
